...
export_schema_to_data_repo <-
        function(conn,
                ...
                importedData <-
                        tables %>%
                        rubix::map_names_set(function(x) if (exists(x, envir = environment())) {
...
#' @title
#' Export a skyscraper Schema to its Data Package Repository
#'
#' @inherit local_maintenance_functions description
#'
#' @inheritSection local_maintenance_functions Exporting Schemas
...
export_schema <-
        function(conn,
                ...
                # Load package datasets to merge with the local datasets
                importedData <-
                        list.files(data_raw_path, full.names = TRUE, pattern = "[.]{1}csv$") %>%
                ...
                names(importedData) <- stringr::str_replace_all(basename(names(importedData)),
                                                                pattern = "(^.*?)([.]{1}csv$)",
                                                                replacement = "\\1"
                                                                )
                ############
                ...
                declareObjsLines <-
                        names(mergedData2) %>%
                        purrr::map2(paste0("data-raw/", basename(data_raw_paths)),
                                    function(x, y) paste0(x, " <- readr::read_csv('",y, "')")) %>%
                        unlist()

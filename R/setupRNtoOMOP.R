#' @title
#' Transform the RN Tables into OMOP Vocabulary Tables
#'
#' @description
#' This function writes the first Concept, Concept Synonym, Concept Ancestor, and Concept Relationship Tables from the Classification, Synonyms, and Phrase Log Tables in the chemidplus schema. If the Concept, Concept Synonym, Concept Ancestor, and/or Concept Relationship Tables already exist in the chemidplus Schema, an error will be thrown.
...
setupRNtoOMOP <-
        function(conn) {
                ...
                # Pair Phrase_Log back with concept_id
                # 1. Get unique input, type and rn combinations. Type will end up being the concept_class_id.
                # 2. Join with HemOnc Vocabulary (Concept Table) that are valid, in the Drug domain
                # 3. Create concept_table input and filter out any concept_id == NA
                # 4. If Concept Table does not already exist in the chemidplus schema, to write a table. Otherwise append the existing concept tabe.
                ...
                pg13::readTable(conn = conn,
                        ...
                        dplyr::filter(no_record == "FALSE") %>%
                        dplyr::select(input,rn,type) %>%
                        dplyr::distinct() %>%
                        dplyr::filter(!is.na(rn))

                # 2. HemOnc concept_ids and concept_name
                hemonc <-
